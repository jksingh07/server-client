...
void getfiles(int client_fd, char **args, int argLen)
{
    printf("%d\n", argLen);
    char *file_list = "";
    ...
    for (int i = 1; i < argLen; i++)
    {
        if (args[i] != NULL)
        {
            char *file = args[i];
            printf("%s 11\n", file);
            // if (access(strcat("~/", file), F_OK) != -1) {  // Check if file exists
            char *temp = malloc(strlen(file_list) + strlen(file) + 1);
    ...
    fileptr = fopen("temp.tar.gz", "rb"); // Open the file in binary mode
    ...
    send(client_fd, buffer, filelen, 0); // Send the compressed file to client
...
void dgetfiles(int client_sockfd, char **arguments)
{
    ...
    char *root_path = "/Users/jaskaransingh/source";
...
void sget(char *root_path, size_t size1, size_t size2, char *command, char *files)
{
    int files_added = 0;
    char buffer[MAX_BUFFER_SIZE];
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        {
            sget(path, size1, size2, command, files);
        }
    }
    closedir(dir);
}
...
void sgetfiles(int client_sockfd, char **arguments)
{
    ...
    sget(root_path, size1, size2, command, files);
    replace(files);
    sprintf(buffer, command, files);
    // printf("%s", buffer);
    if (strlen(files) != 0)
